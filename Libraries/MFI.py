...
def MFI_2D(HILLS="HILLS", position_x="position_x", position_y="position_y", bw=1, kT=1,
	...
	for i in range(total_number_of_hills):
		...
		for j in range(len(periodic_images)):
			...
			#Fbias_x = Fbias_x + height_meta * kernelmeta * ((X) / sigma_meta2_x);
		...
		if FES_cutoff > 0: 
			if (i + 1) % int(error_pace) == 0 or (i+1) == total_number_of_hills:
				...
				else: [X, Y, FES] = intgrad2(Ftot_x_temp, Ftot_y_temp, per1 = False, per2 = False, min_grid=min_grid, max_grid=max_grid)
...
def intgrad2(fx, fy, nx=0, ny=0, intconst=0, per1 = False, per2 = False, min_grid=np.array((-2, -2)), max_grid=np.array((2, 2)), nbins=0):
	...
	#Trailing edge: last point of the domain
	for j in range(0,nx):
...
def bootstrap_2D(X, Y, forces_all, n_bootstrap, FES_cutoff = 0, FFT_integration=0, min_grid=np.array((-3, -3)), max_grid=np.array((3, 3))):
	...
	for iteration in range(n_bootstrap):
		...
		else: [X, Y, FES] = intgrad2(Ftot_x, Ftot_y, per1=False, per2=False, min_grid=min_grid, max_grid=max_grid)
...
def bootstrap_2D_fes(X, Y, dx, dy, forces_all, n_bootstrap, FES_cutoff = 0, FFT_integration=0, min_grid=np.array((-3, -3)), max_grid=np.array((3, 3))):
	...
	#Patch forces
	[Ftot_den, Ftot_x, Ftot_y] = patch_2D_simple(forces_all, nbins=((200,200)))
	...
	for iteration in range(n_bootstrap):
		...
		#patch forces to find average Ftot_den, Ftot and FES
		[Ftot_den, Ftot_x, Ftot_y] = patch_2D_simple(force_rand_select,nbins=((200,200)))
		
		#Calculate FES. if there is a FES_cutoff, find cutoff. 
		if FFT_integration == 1: [X, Y, FES] = FFT_intg_2D(Ftot_x, Ftot_y, min_grid=min_grid, max_grid=max_grid)
		else: [X, Y, FES] = intgrad2(Ftot_x, Ftot_y, 200, 200,0,False,False, min_grid, max_grid)
